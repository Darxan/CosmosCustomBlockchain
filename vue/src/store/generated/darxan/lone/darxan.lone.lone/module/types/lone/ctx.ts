...
export const Ctx = {
  encode(message: Ctx, writer: Writer = Writer.create()): Writer {
    if (message.creator !== "") {
  ...
  decode(input: Reader | Uint8Array, length?: number): Ctx {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseCtx } as Ctx;
  ...
  fromPartial(object: DeepPartial<Ctx>): Ctx {
    const message = { ...baseCtx } as Ctx;
    if (object.creator !== undefined && object.creator !== null) {
