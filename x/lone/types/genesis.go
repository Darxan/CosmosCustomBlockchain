...
func (gs GenesisState) Validate() error {
	...
	customtxIndexMap := make(map[string]struct{})

	for _, elem := range gs.CustomtxList {
		index := string(CustomtxKey(elem.Index))
		if _, ok := customtxIndexMap[index]; ok {
			return fmt.Errorf("duplicated index for customtx")
		}
		customtxIndexMap[index] = struct{}{}
	}
